...
class StreamlitDebugRenderingTests(unittest.TestCase):
    ...
    def test_inject_application_theme_includes_bottom_chat_composer_rules(self) -> None:
        ...
        self.assertIn("max-width: min(56rem, calc(100vw - (1rem * 2)))", injected_theme)
        ...
        self.assertIn("const chatComposerMaxWidthPixels = 896", injected_theme)
        ...
        self.assertIn("const availableWidth = Math.max(0, safeRight - safeLeft)", injected_theme)
        self.assertIn("const viewportCenterX = window.innerWidth / 2", injected_theme)
        self.assertIn("const availableLeftHalfWidth = Math.max(0, viewportCenterX - safeLeft)", injected_theme)
        self.assertIn("const availableRightHalfWidth = Math.max(0, safeRight - viewportCenterX)", injected_theme)
        self.assertIn("const centeredWidthLimit = Math.max(", injected_theme)
        self.assertIn("Math.min(availableLeftHalfWidth, availableRightHalfWidth) * 2", injected_theme)
        self.assertIn("const safeWidth = Math.min(", injected_theme)
        self.assertIn("centeredWidthLimit", injected_theme)
        self.assertIn("const centeredSafeLeft = viewportCenterX - (safeWidth / 2)", injected_theme)
        self.assertIn('"--jenrag-chat-composer-left",\n            `${centeredSafeLeft}px`', injected_theme)
