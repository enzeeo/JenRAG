...
class StreamlitDebugRenderingTests(unittest.TestCase):
    ...
    def test_inject_application_theme_includes_bottom_chat_composer_rules(self) -> None:
        ...
        self.assertIn("const safeWidth = Math.min(availableWidth, chatComposerMaxWidthPixels)", injected_theme)
        self.assertIn("const centeredSafeLeft = safeLeft + Math.max(0, (availableWidth - safeWidth) / 2)", injected_theme)
